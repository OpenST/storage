...
const createTestCasesForOptions = function(optionsDesc, options, toAssert, dataToAssert) {
  ...
  it(optionsDesc, async function() {
    ...
    if (options.allocated_shard) {
      await shardManagementObject.configureShard({
        shard_name: shardName,
        allocation_type: availableShardConst.enabled
      });
    }

    const response = await shardManagementObject.assignShard({
      identifier: identifier,
      entity_type: entityType,
      shard_name: shardName,
      force_assignment: forceAssignment
    });
...
describe('services/dynamodb/shard_management/managed_shard/assign_shard', function() {
  ...
  createTestCasesForOptions(
    'Assign shard having invalid shard name',
    {
      invalidShardName: true
    },
    false,
    's_sm_ms_as_validateParams_3'
  );

  createTestCasesForOptions(
    'Assign shard having invalid identifier',
    {
      invalidIdentifier: true
    },
    false,
    's_sm_ms_as_validateParams_1'
  );

  createTestCasesForOptions(
    'Assign shard having invalid entity type',
    {
      inValidEntityType: true
    },
    true,
  ...
  createTestCasesForOptions(
    'Assign shard having undefined force assignment',
    {
      undefined_force_assignment: true
    },
    false,
    's_sm_ms_as_validateParams_4'
  );

  createTestCasesForOptions(
    'Assign shard having undefined force assignment and allocated shard true',
    {
      undefined_force_assignment: true,
