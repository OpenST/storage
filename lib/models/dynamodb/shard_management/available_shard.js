...
AvailableShard.prototype = {
  ...
  configureShard: function (params) {
    ...
    return ddbObject.queryDdb('updateItem', updateItemParam, 'dax');
  },
  ...
  getShardByName: async function (params) {
    ...
    ;
    const response = await ddbObject.queryDdb('query', queryParams, 'dax');
  ...
  getShardsByEntityAllocation: async function (params) {
    ...
    try {
      for (let ind = 0; ind < ids.length; ind++) {
        ...
        promiseArray.push(ddbObject.queryDdb('query', queryParams, 'dax'));
      }
...
const Item = function (paramDdbItem) {
  ...
  defineProperty(oThis, "shardName",
    ddbItem[availableShardConst.SHARD_NAME]['S']
  );

  defineProperty(oThis, "entityType",
    ddbItem[availableShardConst.ENTITY_TYPE]['S']
  );

  defineProperty(oThis, "allocationType",
    availableShardConst.getInverseShardTypes()[ddbItem[availableShardConst.ALLOCATION_TYPE]['N']]
  );

  defineProperty(oThis, "createdAt",
    ddbItem[availableShardConst.CREATED_AT]['N']
  );

  defineProperty(oThis, "updatedAt",
    ddbItem[availableShardConst.UPDATED_AT]['N']
  );
};

Item.prototype.constructor = Item;

InstanceComposer.register(AvailableShard, 'getDDBServiceAvailableShard');
