...
const tokenBalanceModelSpecificPrototype = {
  ...
  getBalance: async function (params) {
    ...
    for (let i = 0; i < params['ethereum_addresses'].length; i ++) {
      params['ethereum_addresses'][i] = params['ethereum_addresses'][i].toLowerCase();
      queryKeys.push(oThis._keyObj({ethereum_address: params['ethereum_addresses'][i]}))
    }
