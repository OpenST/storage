...
const transactionLogModelSpecificPrototype = {
  ...
  batchPutItem: async function (rawData) {
    ...
    while (true) {
      ...
      console.log("dataBatchNo = ", dataBatchNo, "batchedrawData length = ", batchedrawData.length);
      ...
      if (allPromisesData.length == parallelPromisesCount || (batchedrawData.length == 0 && allPromisesData.length > 0)) {
        ...
        console.log("allPromisesData.length = ", allPromisesData.length);
        ...
        for (let i = 0; i < promiseResponses.length; i++) {
          if (promiseResponses[i].isFailure()) {
            ...
            if (Object.keys(unprocessedItems).length > 0) {
              logger.error(`error batchPutItem clientId : ${oThis.clientId} batch : ${formattedErrorCount} promiseNo : ${i} unprocessedItems : ${unprocessedItems[oThis.shardName].length}`);
