...
const transactionLogModelSpecificPrototype = {
  ...
  batchPutItem: async function(rawData) {
    ...
    while(true) {
      ...
      allPromises.push(oThis.ddbServiceObj.batchWriteItem(batchPutParams));

      dataBatchNo = dataBatchNo + 1;

    }

    let promisesBatchNo = 1;

    while(true) {

      const offset = (promisesBatchNo - 1) * parallelPromisesCount
          , batchedPromises = allPromises.slice(offset, parallelPromisesCount + offset)
      ;

      logger.info(`starting batchPutItem clientId : ${oThis.clientId} batch : ${promisesBatchNo} length : ${batchedPromises.length}`);

      if (batchedPromises.length === 0) break;

      let promiseResponses = await Promise.all(batchedPromises);

      for (let i=0; i<promiseResponses.length; i++) {
        if (promiseResponses[i].isFailure()) {
          formattedErrorPromiseResponses[formattedErrorCount] = promiseResponses[i].toHash();
        } else {
          let unprocessedItems = promiseResponses[i].data.UnprocessedItems;
          if(Object.keys(unprocessedItems).length > 0) {
            logger.info(`error batchPutItem clientId : ${oThis.clientId} batch : ${promisesBatchNo} promiseNo : ${i} unprocessedItems : ${unprocessedItems[oThis.shardName].length}`);
            formattedErrorPromiseResponses[formattedErrorCount] = {unProcessedCount: unprocessedItems[oThis.shardName].length};
          }
        }
        formattedErrorCount += 1;
      }

      promisesBatchNo = promisesBatchNo + 1;
